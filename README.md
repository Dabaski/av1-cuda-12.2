...
- **l6_pipeline** — 4x4 block and frame composition:
  ...
  (project-defined policy, SVT primitives); `encodeFrameAuto4x4`
  drives full frames — each block's mode chosen against RECONSTRUCTED
  edges, the chosen modes feeding `NeighborContext` (filt_type live).
  GPU path `predict_block_4x4` + `subtract_4x4_plane` +
  `fwd_txfm_2d_4x4` + `inv_txfm_2d_add_4x4`, bit-exact vs host;
  the GPU frame-auto loop (D4) reproduces the host D3 path exactly.
