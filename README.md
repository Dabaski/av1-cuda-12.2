...
- **l7_entropy** — the entropy coder, host port, integer only,
  ...
  (out-of-tree arbiter; no aom code extracted). Deferred: chroma
  uv_mode/CFL symbols, nonkey y-mode path, tx_type, palette, intrabc,
...
- **l8_bitstream** — raw-bit writer + OBU container ground floor, host
  ...
  U/V quantizer delta writes skipped for mono):
  `writeSequenceHeaderObu` (payload incl. trailing bits),
  `writeFrameHeader` (the ratified 21-bit uncompressed-header walk, no
  trailing-bits marker) and `assembleStructuralKeyframeTU` (TD + SPS
  with phase-1 measure / phase-2 uleb rewrite + OBU_FRAME, tile-group
  header 0 bytes at tile_cnt == 1, dst zeroed first so the pad bits
  after the header are the spec's byte_alignment zeros). Milestone
  artifact `src/l8_bitstream/tests/goldens/structural_keyframe.obu`
  (23 bytes, produced from the generator output): the test proves the
  three-way identity composed TU == committed file == gate bytes.
...
src/
  ...
  l7_entropy/     entropy coding: od_ec range coder, CDF adaptation,
                  partition/skip/kf-mode/filter-intra symbols (host)
  l8_bitstream/   raw-bit writer + OBU ground floor, structural keyframe
                  TU assembly (+ committed 23-byte .obu golden artifact)
tools/
