...
- **l8_bitstream** — raw-bit writer + OBU container ground floor, host
  ...
  the spec's byte_alignment zeros). BSF4-fix ec coupling, stated as an
  ...
  (23 bytes, produced from the generator output, regenerated by the
  fix): the test proves the three-way identity composed TU ==
  committed file == gate bytes, and the TU is decoder-accepted —
  libdav1d decodes it silently (exit 0), ffprobe reports
  codec_name=av1, 32x32, pix_fmt=gray, color_range=pc.
